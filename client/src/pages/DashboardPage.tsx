...
export const DashboardPage = () => {
  ...
  return (
    <div className="flex flex-col gap-6 sm:gap-10 py-6 sm:py-10 animate-fade-in text-left">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 sm:gap-6 border-b border-slate-200 pb-6 sm:pb-10">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2 mb-1 sm:mb-2">
             <div className="w-2 h-2 rounded-full bg-blue-600 animate-pulse"></div>
             <p className="text-blue-700 font-black uppercase tracking-widest text-[10px] sm:text-xs bg-blue-50 py-1 px-3 rounded-full">Vrundavan Society Portal</p>
          </div>
          <h1 className="text-2xl sm:text-4xl md:text-5xl font-black tracking-tight text-slate-900">
            Hello, {user?.name || 'Resident'} 👋
          </h1>
          <p className="text-slate-600 font-semibold text-sm sm:text-lg mt-0.5 sm:mt-1">
        ...
        <div className="flex gap-3 w-full md:w-auto">
           <button 
             ...
             className="w-full md:w-auto btn btn-primary px-6 sm:px-8 py-3.5 sm:py-4 shadow-xl shadow-blue-200 text-sm sm:text-base font-bold active:scale-95"
           >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-10">
        <div className="lg:col-span-2 flex flex-col gap-6">
          <div className="flex justify-between items-center">
            <h2 className="text-xl sm:text-2xl font-black tracking-tight text-slate-900">Member Services</h2>
            <Link to="/facilities" className="text-xs sm:text-sm font-bold text-blue-600 hover:underline">Explore all services</Link>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
             {quickActions.map((action, i) => (
               <Link key={i} to={action.path} className="text-decoration-none group">
                 <GlassCard className="flex flex-col items-center text-center gap-2 sm:gap-3 py-5 sm:py-8 hover:bg-slate-50 border-slate-200 transition-all h-full shadow-sm">
                    ...
                    <div>
                      <span className="font-extrabold text-slate-900 text-sm sm:text-base block">{action.name}</span>
                      <span className="text-[10px] sm:text-xs text-slate-500 uppercase font-bold tracking-wider mt-0.5 sm:mt-1 block">{action.desc}</span>
                    </div>
                 </GlassCard>
               </Link>
             ))}
          ...
          <div className="flex flex-col gap-4">
               ...
               <div className="p-6 sm:p-8 bg-gradient-to-br from-blue-700 to-blue-900 text-white border-none shadow-blue-200 shadow-2xl rounded-3xl sm:rounded-[3rem] relative overflow-hidden group">
                  ...
                  <div className="relative z-10">
                    <h4 className="font-extrabold text-lg sm:text-xl tracking-tight">Need Assistance?</h4>
                    ...
                    <button 
                       ...
                       className="mt-6 sm:mt-8 w-full py-3 sm:py-4 bg-white text-blue-700 font-black uppercase tracking-wider rounded-xl sm:rounded-2xl text-xs sm:text-sm hover:translate-y-[-2px] transition-all shadow-xl shadow-black/10 active:scale-95"
                    >
                       Contact Committee
                    </button>
                  </div>
