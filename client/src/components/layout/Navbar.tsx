...
export const Navbar = () => {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-[100] bg-white/95 backdrop-blur-md border-b border-slate-100 shadow-sm">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 sm:h-20 items-center justify-between">
            <div className="flex items-center gap-4 sm:gap-6 lg:gap-12">
              <Link
                to="/"
                className="text-xl sm:text-2xl font-black tracking-tighter text-slate-900 group flex items-center gap-2 text-decoration-none"
              >
                <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-blue-600 flex items-center justify-center text-white text-xs rotate-12 group-hover:rotate-0 transition-all duration-500 font-black">
                  BH
                </div>
                <span>
              ...
              <div className="hidden lg:flex items-center gap-1 xl:gap-2">
                {navLinks.map((link) => (
                  <Link
                    key={link.to}
                    to={link.to}
                    className={`flex items-center gap-2 px-3 xl:px-5 py-2.5 rounded-xl text-sm font-extrabold transition-all text-decoration-none ${
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="relative" ref={notifRef}>
                <button
                  ...
                  className={`w-9 h-9 sm:w-11 sm:h-11 rounded-xl sm:rounded-2xl flex items-center justify-center transition-all border relative cursor-pointer ${
                    showNotifs
                ...
                >
                  ...
                  {unreadCount > 0 && (
                    <span className="absolute top-[-2px] right-[-2px] w-4 h-4 sm:w-5 sm:h-5 bg-red-600 text-white text-[9px] sm:text-[10px] font-black rounded-full flex items-center justify-center border-2 border-white shadow-sm">
                      {unreadCount}
                    </span>
                  )}
                </button>
                      ...
                      <div className="flex items-center gap-2">
                        {unreadCount > 0 && (
                          <button
                            type="button"
                            onClick={markAllRead}
                            className="text-[10px] font-extrabold text-blue-600 hover:text-blue-800 underline uppercase tracking-wider cursor-pointer"
                          >
                            Mark read
                          </button>
                        )}
                    ...
                    <div className="max-h-[350px] overflow-y-auto flex flex-col">
                      {notifications.length > 0 ? (
                        notifications.map((notif, i) => (
                          <div
                            key={notif._id || i}
                            className={`p-4 sm:p-5 hover:bg-slate-50 transition-colors cursor-pointer border-b border-slate-100 last:border-0 relative ${
              ...
              <div className="hidden lg:flex items-center gap-3">
                <Link
                  to="/profile"
                  className="flex items-center gap-2 pl-3 border-l border-slate-200 text-decoration-none group"
                  title="My Profile"
                >
                ...
                <button
                  ...
                  className="w-11 h-11 rounded-2xl flex items-center justify-center text-red-600 hover:text-white hover:bg-red-600 transition-all border border-red-200 cursor-pointer"
                  title="Logout"
                >
                  <LogOut size={20} />
                </button>
              ...
              <button
                type="button"
                className="lg:hidden text-slate-900 p-2 sm:p-2.5 hover:bg-slate-100 active:bg-slate-200 rounded-xl focus:outline-none cursor-pointer relative z-[110] flex items-center justify-center min-w-[36px] min-h-[36px]"
                onClick={(e) => {
                  e.stopPropagation();
                  toggleMenu();
                }}
      ...
      {/* Mobile Fullscreen Drawer (Rendered at top-level for clean z-index stacking) */}
      {isOpen && (
        <div className="lg:hidden fixed top-16 sm:top-20 inset-x-0 bottom-0 bg-white/98 backdrop-blur-2xl border-t border-slate-200 overflow-y-auto z-[999] shadow-2xl animate-fade-in">
          <div className="flex flex-col p-4 sm:p-6 gap-2 pb-24">
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`flex items-center gap-3.5 p-3.5 sm:p-4 rounded-xl sm:rounded-2xl font-extrabold text-sm transition-all text-decoration-none ${
                  location.pathname === link.to
            ...
            {user.role === 'ADMIN' && (
              <>
                <div className="text-xs font-black uppercase tracking-wider text-amber-700 px-3 pt-3">
                  Administration
                </div>
                <Link
                  to="/admin"
            ...
            <Link
              to="/profile"
              className={`flex items-center gap-3.5 p-3.5 sm:p-4 rounded-xl font-extrabold text-sm text-decoration-none ${
                location.pathname === '/profile' ? 'bg-blue-600 text-white' : 'text-slate-800 hover:bg-slate-100'
              }`}
              onClick={() => setIsOpen(false)}
            >
            ...
            <button
              ...
              className="flex items-center gap-3.5 p-3.5 sm:p-4 rounded-xl text-red-600 font-extrabold text-sm hover:bg-red-50 text-left w-full mt-2 cursor-pointer"
            >
